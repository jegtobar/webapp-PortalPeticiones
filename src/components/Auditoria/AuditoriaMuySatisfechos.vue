<template>
    <v-container>
          <template>
            <v-row>
                <v-col class="mb-4">
                    <h1 class="display-1 text-left font-weight-bold mb-3">Auditoría: Mantenimiento Peticiones Vecinos Muy Satisfechos</h1>
                </v-col>
            </v-row>
...
import { mapMutations } from 'vuex';
  ...
  export default {
    ...
    methods: {
      ...
      getAlcaldias(){
        ...
        getPersonas(){
            axios.get(process.env.VUE_APP_SERVICE_URL+'persona/listaMntoMuySatisfechosAuditoria/'+this.alcaldias.alcaldia_id)
                .then((response)=>{
 ...
 ...mapMutations({
        ...
        obtener_datos(){
          ...
          axios.get(process.env.VUE_APP_SERVICE_URL+'reporte/mntomuysatisfechos/'+ this.alcaldias.alcaldia_id)
                .then((response)=>{
        ...
        auditarSeguimiento(item){
                this.realizado = Object.assign({}, item)
                 this.$swal({
                    ...
                    axios.put(process.env.VUE_APP_SERVICE_URL+'gestion/mntoseguimientomuysatisfecho/actualizar/'+this.realizado.id, this.actRealizado)
                    .then(()=>{
                      this.buscarSeguimientoVecino()
