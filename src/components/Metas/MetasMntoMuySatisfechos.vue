<template>
    <v-container>
        <template>
            <v-row>
                <v-col class="mb-4">
                    <h1 class="display-1 text-left font-weight-bold mb-3">Metas Mantenimiento Vecinos Muy Satisfechos</h1>
                     <v-select
                ...
                <template v-slot:item.actual="{ item }">
                  ...
                  <template v-slot:top>
                    <v-toolbar
                      flat
                    >
                      <v-toolbar-title>Lista de Metas</v-toolbar-title>
                      <v-divider
                        class="mx-4"
                        inset
                        vertical
...
import axios from 'axios';
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Nueva Meta' : 'Editar Meta'
      },
    },
    ...
    watch: {
     ...
     getPromotor(){
                axios.get(process.env.VUE_APP_SERVICE_URL+'user/promotorcoordinador/'+this.alcaldias.alcaldia_id)
                .then((response)=>{
      ...
      getColonias(){
          axios.get(process.env.VUE_APP_SERVICE_URL+'catalogo/coloniasporzona/'+this.alcaldias.alcaldia_id)
                .then((response)=>{
      ...
      getMetas(){
          axios.get(process.env.VUE_APP_SERVICE_URL+'metas/mntomuysatisfechos/'+this.alcaldias.alcaldia_id)
                .then((response)=>{
      ...
      guardarMeta(){
        ...
        axios.post(process.env.VUE_APP_SERVICE_URL+'metas/mntomuysatisfechos/crear', this.newItem)
            .then((response)=>{
              if (response.data.res){
                this.$swal('Ya existe una meta en el lugar seleccionado')
                this.limpiar();
              }else{
                this.$swal({
                    title:"Nueva Meta",
      ...
      editarMeta(){
            axios.put(process.env.VUE_APP_SERVICE_URL+'metas/mntomuysatisfechos/actualizar/'+this.editedItem.id, this.editedItem)
            .then(
                 this.$swal({
                    title:"Editar Meta",
