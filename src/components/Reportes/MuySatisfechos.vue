            ...
            switch (JSON.parse(localStorage.getItem('rol'))) {
                case 1:
                  if (JSON.parse(localStorage.getItem('distrito'))===1||JSON.parse(localStorage.getItem('distrito'))===2||JSON.parse(localStorage.getItem('distrito'))===3){
                        axios.get(process.env.VUE_APP_SERVICE_URL+'reporte/seguimientomuysatisfechos/'+JSON.parse(localStorage.getItem('alcaldia'))+'/'+JSON.parse(localStorage.getItem('distrito')))
                        .then((response)=>{
                       ...
                       axios.get(process.env.VUE_APP_SERVICE_URL+'reporte/seguimientomuysatisfechos/'+JSON.parse(localStorage.getItem('alcaldia')))
                        .then((response)=>{
                            this.seguimientos=response.data.reportes;
                            this.titulo=response.data.titulo;
                        })
                    ...
                    break;
                case 4:
                    axios.get(process.env.VUE_APP_SERVICE_URL+'reporte/seguimientomuysatisfechos')
                    .then((response)=>{
                        this.seguimientos=response.data.reportes;
                        this.titulo=response.data.titulo;
                    })
