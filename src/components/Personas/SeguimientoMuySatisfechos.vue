  ...
  <v-container>
    <v-window v-model="step">
          ...
          <v-row>
            <v-col class="mb-4">
              <h1 class="display-1 text-left font-weight-bold mb-3">
                Seguimiento atención de solicitudes
              </h1>
              <v-btn tile outlined dark @click="step++" color="success"
                >Mantenimiento atención de solicitudes</v-btn
              >
            </v-col>
            <v-col v-if="descarga == 'S'">
              <MuySatisfechos />
              <SinMuySatisfechos />
            </v-col>
          </v-row>
              ...
              <template v-slot:top>
                <v-toolbar flat>
                  <v-toolbar-title
                    ...
                    v-on:click="getPersonas()"
                    >Vecinos</v-toolbar-title
                  >
                    ...
                    <v-card>
                      <v-card-title>
                        <v-toolbar dark color="#10069f">
                          ...
                          <v-toolbar-title
                            ><span class="text-h5"
                              >Ingresar Seguimiento</span
                            ></v-toolbar-title
                          >
                          <v-spacer></v-spacer>
                          <v-toolbar-items>
                            <v-btn color="success" @click="putMantenimiento">
                              Trasladar
                            </v-btn>
                          </v-toolbar-items>
                        </v-toolbar>
                          ...
                          >
                            <v-row>
                              ...
                              <v-col cols="12" sm="6" md="4">
                                <v-select
                                  v-model="
                                    seguimientoVecinosSatisfechos.actividad_id
                                  "
                                  :items="actividades"
                                  item-text="actividad"
                                  item-value="id"
                              ...
                              <v-col cols="12" sm="6" md="4">
                                <v-textarea
                                  v-model="
                                    seguimientoVecinosSatisfechos.descripcion
                                  "
                                  label="Descripción"
                                  :rules="Rules"
                                ></v-textarea>
                              ...
                              <v-col cols="12" sm="6" md="4">
                                <v-select
                                  v-model="
                                    seguimientoVecinosSatisfechos.responsable
                                  "
                                  label="Responsable"
                                  :items="promotor"
                                  item-text="promotor"
      ...
      <v-window-item :value="2">
        <v-btn tile outlined dark @click="step--, getPersonas()" color="success"
          >Seguimiento Vecinos Muy Satisfechos</v-btn
        >
...
import SinMuySatisfechos from "../Reportes/SinSeguimientos/MuySatisfechos";
...
export default {
  ...
  methods: {
    getPersonas() {
      if (
        JSON.parse(localStorage.getItem("username")) == "jfigueredo" ||
        JSON.parse(localStorage.getItem("username")) == "jgarcia" ||
        JSON.parse(localStorage.getItem("rol")) === 1
      ) {
        this.descarga = "S";
      }
      this.loading = true;
      if (
        JSON.parse(localStorage.getItem("rol")) === 4 ||
        JSON.parse(localStorage.getItem("rol")) === 6
      ) {
        axios
          .get(process.env.VUE_APP_SERVICE_URL + "persona/listaMuySatisfechos")
          .then((response) => {
        ...
        axios
          .get(
            process.env.VUE_APP_SERVICE_URL +
              "persona/listamuysatisfechoszonadistrito/" +
        ...
        case "1": //Alcaldes auxiliares
          if (
            ...
            axios
              .get(
                process.env.VUE_APP_SERVICE_URL +
                  "persona/searchbyalcaldiazona/3/" +
                  JSON.parse(localStorage.getItem("alcaldia")) +
                  "/" +
                  this.search.split(" ").join("_")
              )
    ...
    addSeguimiento(item) {
      ...
      this.seguimientoVecinosSatisfechos.persona_id =
        this.seguimientoVecinosSatisfechos.id; //Asignacion del id al campo personas_id
      this.seguimientoVecinosSatisfechos.usuario_creador = JSON.parse(
    ...
    guardarSeguimiento() {
      ...
      if (this.validSeguimiento) {
        axios
          .post(
            process.env.VUE_APP_SERVICE_URL +
              "gestion/seguimientomuysatisfecho/create",
