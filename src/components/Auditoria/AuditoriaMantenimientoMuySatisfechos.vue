<template>
    <v-container>
          <template>
            <v-row>
                <v-col class="mb-4">
                    <h1 class="display-1 text-left font-weight-bold mb-3">Auditoría: Mantenimiento Vecinos Muy Satisfechos</h1>
                </v-col>
            </v-row>
  ...
  export default {
    ...
    methods: {
      ...
      getAlcaldias(){
        ...
        getPersonas(){
            axios.get(process.env.VUE_APP_SERVICE_URL+'persona/listaMantenimientoMuySatisfechos/'+this.alcaldias.alcaldia_id)
                .then((response)=>{
