  ...
  <v-container>
    ...
    <v-row>
      <v-col class="mb-4">
        ...
        <v-card-actions>
          <v-btn color="primary" class="mr-4" @click="buscarVecino()"
            >Buscar</v-btn
          >
        </v-card-actions>
      </v-col>
            ...
            <v-list-item three-line>
              <v-list-item-content>
                ...
                <v-list-item-subtitle
                  ><strong>DPI:</strong> {{ vecino.dpi }}</v-list-item-subtitle
                >
                <v-list-item-subtitle
    ...
    <br />
    <v-form ref="form" v-model="valid" @submit.prevent="guardarPeticion">
      <v-row>
        <v-col>
          <v-text-field
          ...
          <v-text-field
            v-model="peticion.oficio"
            label="Oficio"
            outlined
            color="#10069f"
          ></v-text-field>
          <v-select
            v-model="peticion.peticion_id"
            :items="tipoPeticion"
            item-text="peticion"
            item-value="id"
        ...
        <v-col>
          ...
          <v-select
            ...
            color="#10069f"
            @change="
              getDistrito(), getSector(), getColoniaByZona(), getPromotor()
            "
            :rules="Rules"
            required
          ></v-select>
          ...
          ></v-select>
          <v-select
            v-model="peticion.dependencia_id"
            :items="dependencia"
...
export default {
  ...
  methods: {
    getCanales() {
      axios
        .get(process.env.VUE_APP_SERVICE_URL + "catalogo/canales")
        .then((response) => {
    ...
    getColoniaByDistrito() {
      axios
        .get(
          process.env.VUE_APP_SERVICE_URL +
            "catalogo/coloniaspordistrito/" +
            this.peticion.distrito +
            "/" +
            this.peticion.zona
        )
        .then((response) => {
    ...
    getColoniaBySector() {
      axios
        .get(
          process.env.VUE_APP_SERVICE_URL +
            "catalogo/coloniasporsector/" +
            this.peticion.sector +
            "/" +
            this.peticion.zona
        )
        .then((response) => {
    ...
    getDependencia() {
      axios
        .get(process.env.VUE_APP_SERVICE_URL + "catalogo/dependencia")
        .then((response) => {
    ...
    getTipoPeticion() {
      axios
        .get(process.env.VUE_APP_SERVICE_URL + "catalogo/peticiones")
        .then((response) => {
    ...
    guardarPeticion() {
      ...
      if (this.valid) {
        axios
          .post(
            process.env.VUE_APP_SERVICE_URL + "gestion/register",
            this.peticion
          )
          .then((response) => {
            this.$swal({
