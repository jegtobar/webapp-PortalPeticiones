...
<template>
   <v-row>
      <v-col class="mb-4">
        <h1 class="display-1 text-left font-weight-bold mb-3">Peticiones por Vecino</h1>
      </v-col>
    </v-row>
    <v-row>
        <v-col class="mb-4">
        <v-text-field 
        label="Ingrese DPI del Vecino o número de celular" v-model="persona.dpi"
        >
        </v-text-field>
        <v-card-actions>
         <v-btn color="primary" class="mr-4" @click="buscarVecino(), buscarPeticionesVecino()">Buscar</v-btn>
        </v-card-actions>
        </v-col>
          <v-col class="mb-4">
              <v-col>
        ...
        >
          ...
          <!-- <v-toolbar-items>
            ...
            <v-card-text>
              <v-container>
                <v-list-item-content>
                    <v-list-item-title>Descripción de la petición:</v-list-item-title>
                    <v-list-item-subtitle>{{editedItem.descripcion}}</v-list-item-subtitle>
                </v-list-item-content>
                  <!-- <v-textarea
                ...
                <v-row>
                 ...
                 <v-divider></v-divider>
                    ...
                    <v-list-item-content>
                        <v-list-item-title>Historial seguimiento de la petición</v-list-item-title>
                    </v-list-item-content>
                    <template>
                        <v-data-table
    ...
    methods: {
      ...
      guardarUsuario(){
        ...
        buscarPeticionesVecino(){
            axios.get(process.env.VUE_APP_SERVICE_URL+'gestion/vecino/'+this.persona.dpi+'/'+JSON.parse(localStorage.getItem('alcaldia')))
            .then((response)=>{
        ...
        buscarGestionSeguimiento(){
            axios.get(process.env.VUE_APP_SERVICE_URL+'gestion/seguimiento/'+this.editedItem.id)
            .then((response)=>{
