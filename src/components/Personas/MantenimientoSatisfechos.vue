      ...
      <v-row>
        <v-col class="mb-4">
          <h1 class="display-1 text-left font-weight-bold mb-3">
            Mantenimiento atención de solicitudes
          </h1>
        </v-col>
...
import SinMntoSatisfechos from "../Reportes/SinSeguimientos/MntoSatisfechos.vue";
...
export default {
  ...
  methods: {
    getPersonas() {
      ...
      if (
        ...
        axios
          .get(
            process.env.VUE_APP_SERVICE_URL +
              "persona/listaMantenimientoSatisfechos"
          )
          .then((response) => {
        ...
        axios
          .get(
            process.env.VUE_APP_SERVICE_URL +
              "persona/mntozonadistritosatisfechos/" +
        ...
        case "1": //Alcaldes auxiliares
          if (
            ...
            axios
              .get(
                process.env.VUE_APP_SERVICE_URL +
                  "persona/searchbyalcaldiazona/2/" +
                  JSON.parse(localStorage.getItem("alcaldia")) +
                  "/" +
                  this.search.split(" ").join("_")
              )
    ...
    addSeguimiento(item) {
      ...
      this.mntoVecinosSatisfechos.persona_id =
        this.seguimientoVecinosSatisfechos.id; //Asignacion del id al campo personas_id
      this.mntoVecinosSatisfechos.usuario_creador = JSON.parse(
    ...
    guardarSeguimiento() {
      ...
      if (this.validSeguimiento) {
        axios
          .post(
            process.env.VUE_APP_SERVICE_URL +
              "gestion/mntoseguimientosatisfecho/create",
