<template>
    <v-container>
      <v-window v-model="step">
            ...
            <v-row>
                <v-col class="mb-4">
                    <h1 class="display-1 text-left font-weight-bold mb-3">Seguimiento atención de solicitudes</h1>
                    <v-btn tile outlined dark @click="step++" color="success">Mantenimiento atención de solicitudes</v-btn>
                </v-col>
                <v-col
                v-if="descarga=='S'">
                   <MuySatisfechos/>
...
<!-- Acciones Editar-Eliminar -->
                ...
                <v-btn tile outlined dark @click="step--,getPersonas()" color="success">Seguimiento Vecinos Muy Satisfechos</v-btn>
...
import MantenimientoMuySatisfechos from './MantenimientoMuySatisfechos.vue'
import MuySatisfechos from '../Reportes/MuySatisfechos.vue'
  ...
  export default {
    ...
    colonias:[],
      ...
      newItem: {
        ...
        "genero": "",
        "liderazgo": "",
        "seguimiento":"3", //Seguimiento 1 corresponde a los vecinos satisfechos.
    ...
    methods: {
          ...
          if( JSON.parse(localStorage.getItem('username'))=='jfigueredo' || JSON.parse(localStorage.getItem('username'))=='jgarcia'||JSON.parse(localStorage.getItem('rol'))===1){
           ...
           this.loading=true
            if (JSON.parse(localStorage.getItem('rol'))===4||JSON.parse(localStorage.getItem('rol'))===6){
                axios.get(process.env.VUE_APP_SERVICE_URL+'persona/listaMuySatisfechos')
                .then((response)=>{
            ...
            }else  if (JSON.parse(localStorage.getItem('distrito'))===1||JSON.parse(localStorage.getItem('distrito'))===2||JSON.parse(localStorage.getItem('distrito'))===3)
            {
              axios.get(process.env.VUE_APP_SERVICE_URL+'persona/listamuysatisfechoszonadistrito/'+JSON.parse(localStorage.getItem('alcaldia'))+'/'+JSON.parse(localStorage.getItem('distrito')))
                .then((response)=>{
              ...
              axios.get(process.env.VUE_APP_SERVICE_URL+'persona/listaMuySatisfechos/'+JSON.parse(localStorage.getItem('alcaldia')))
                .then((response)=>{
                    this.personas=response.data.personas
                     this.loading=false
        ...
        vecinosByName(){
                axios.get(process.env.VUE_APP_SERVICE_URL+'persona/lista/3/'+this.search.split(' ').join('_'))
                .then((response)=>{
                    this.search = ""
                    this.personas=response.data.persona
                    this.loading=false
      ...
      editSeguimiento (item) {
       ...
       addSeguimiento(item){
        ...
        this.seguimientoVecinosSatisfechos.persona_id = this.seguimientoVecinosSatisfechos.id //Asignacion del id al campo personas_id
        this.seguimientoVecinosSatisfechos.usuario_creador = JSON.parse(localStorage.getItem('username'))
        ...
        putMantenimiento(){
          this.editedItem.seguimiento = {
            "seguimiento":"1"
                 ...
                 this.$swal({
                    title: '¿Trasladar a Seguimiento Satisfecho?',
                    showDenyButton: true,
                    showCancelButton: true,
                    confirmButtonText: 'Si',
                    denyButtonText: `No`,
